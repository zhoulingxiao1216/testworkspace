...
class Notifier:
    ...
    # =====================================================
    WEB_ITEMS = [
        ("B2B H5站点巡检 　　　 ",    "B2B H5站点标题巡检"),
        ("B2B PC站点巡检 　　　 ",    "B2B PC站点标题巡检"),
        ("D2C PC站点巡检 　　　 ",    "D2C pc站点标题巡检"),
    ]
    API_ITEMS = [
        ("B2B&D2C 登录   　　　",       "登录接口校验"),
        ("B2B&D2C 商品图搜   　　　",   "B2B&D2C 商品图搜接口校验"),
        ("B2B 关键词搜索   　　　",      "B2B 关键字搜索接口校验（1688&淘宝）"),
        ("B2B&D2C 商品加购   　　　",   "B2B&D2C 商品加购接口校验"),
        ("B2B 编辑保存附加项　　　　",  "B2B 选择商品附加项"),
        ("D2C 选择商品附加项　　　　",  "D2C 选择商品附加项"),
        ("B2B&D2C 提交自助报价单　　",  "B2B&D2C 提交自助报价单"),
        ("B2B&D2C 支付报价单　　　　",  "B2B&D2C报价单支付"),
    ...
    def _build_dynamic_template(report_data, is_all_pass):
            ...
            for key, val in report_data.items():
                ...
                if "success" in val:
                    ...
                    for sub_key, sub_val in val.items():
                        if isinstance(sub_val, dict) and "success" in sub_val:
        ...
        # ── 网页访问巡检 ──
        web_executed = [(label, key) for label, key in Notifier.WEB_ITEMS if key in flat_results]
        if web_executed:
            content += "　\n"
            content += "📍 樱花生产站点访问检查\n\n"
            content += "　\n"
            for label, key in web_executed:
                result = flat_results[key]
        ...
        # ── 核心业务巡检 ──
        api_executed = [(label, key) for label, key in Notifier.API_ITEMS if key in flat_results]
        if api_executed:
            content += "　\n"
            content += "💼 核心业务检查\n\n"
            content += "　\n"
