    ...
    try:
        ...
        try:
            ...
            if not b2b_token or not b2b_cookie:
                ...
                response = requests.post(b2b_list_url, json=b2b_list_payload, headers=b2b_list_headers, timeout=25, verify=False, proxies={'http': None, 'https': None})
                ...
                if check["success"]:
                    ...
                    try:
                        ...
                        if target_mail not in orderid_mapping:
                            orderid_mapping[target_mail] = {"B2B": [], "D2C": []}
                        orderid_mapping[target_mail]["B2B"] = orderids
                        with open(orderid_file, "w", encoding="utf-8") as f:
                            json.dump(orderid_mapping, f, ensure_ascii=False, indent=2)
            ...
            # B2B步骤2：支付报价单（每3天支付一次）
            if b2b_token and b2b_cookie:
                # 检查是否应该支付（每3天支付一次）
                if not should_pay(orderid_file, target_mail, "B2B"):
                    msgs.append("B2B报价单支付:跳过(距离上次支付未满3天)")
                else:
                        ...
                        try:
                            with open(orderid_file, "r", encoding="utf-8") as f:
                                ...
                                if target_mail in orderid_mapping:
                                    b2b_orderids = orderid_mapping[target_mail].get("B2B", [])
                        except:
                            pass
                    
                    if not b2b_orderids:
                        msgs.append("B2B报价单支付:FAIL(未找到订单ID)")
                    else:
                        b2b_pay_url = f"{base_url}{endpoints.get('payment_B2B_pay', '')}"
                        b2b_pay_headers = get_b2b_headers(base_url, b2b_token, b2b_cookie)
                        b2b_pay_headers['content-type'] = 'application/json'
                        
                        b2b_pay_payload = payment_payloads.get("B2B_pay", {}).copy()
                        b2b_pay_payload["orderIds"] = b2b_orderids[0]  # 使用第一个订单ID
                        
                        response = requests.post(b2b_pay_url, json=b2b_pay_payload, headers=b2b_pay_headers, timeout=25, verify=False, proxies={'http': None, 'https': None})
                        check = AssertionTool.verify_api_common(response, rules=all_rules.get("B2B_pay", {}))
                        if check["success"]:
                            msgs.append("B2B报价单支付:OK")
                            # 更新支付时间
                            update_pay_time(orderid_file, target_mail, "B2B")
                        else:
                            msgs.append(f"B2B报价单支付:FAIL({check.get('message', '未知错误')})")
        except Exception as e:
        ...
        try:
            ...
            if not d2c_cookie:
                ...
                response = requests.post(d2c_list_url, json=d2c_list_payload, headers=d2c_list_headers, timeout=25, verify=False, proxies={'http': None, 'https': None})
            ...
            if d2c_cookie:
                ...
                if not should_pay(orderid_file, target_mail, "D2C"):
                    ...
                    if not d2c_orderids:
                        ...
                        response = requests.post(d2c_pay_url, json=d2c_pay_payload, headers=d2c_pay_headers, timeout=25, verify=False, proxies={'http': None, 'https': None})
        ...
        def is_step_success(msg):
            ...
            if ":OK" in msg:
                return True
            elif ":跳过" in msg and "距离上次支付未满3天" in msg:
                return True  # 3天间隔跳过视为成功
            elif ":FAIL" in msg:
                return False
            else:
                return False
