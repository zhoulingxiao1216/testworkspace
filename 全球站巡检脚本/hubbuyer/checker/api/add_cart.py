...
def build_payload(task_name, default_pool, rules):
    ...
    # B2B 任务：从数组中随机选择一组参数
    if task_name in ["B2B_1688", "B2B_taobao"]:
        if isinstance(task_data, list):
            if task_data:
                payload = random.choice(task_data).copy()
            else:
                raise ValueError(f"{task_name} 参数数组为空")
        else:
            # 兼容旧格式（单个对象）
...
def build_url(task_name, endpoints, base_url):
    """构建 URL"""
    # B2B 任务使用独立的 endpoint
    if task_name == "B2B_1688":
        endpoint = endpoints.get("add_cart_B2B_1688", "")
    elif task_name == "B2B_taobao":
        endpoint = endpoints.get("add_cart_B2B_taobao", "")
    elif "D2C" in task_name:
        endpoint = endpoints.get("add_cart_D2C", "")
    else:
        endpoint = endpoints.get("add_cart_B2B", "")
        ...
        for task_name in ["B2B_1688", "B2B_taobao"]:
            try:
                ...
                if True:  # 现在只执行 B2B 分支
                    ...
                    # 新格式使用 item_id 字段（旧格式使用 ItemID）
                    item_id = str(payload.get("item_id", payload.get("ItemID", "")))
                    item_url = payload.get("ItemURL", "")
                    ...
                    response = requests.post(url, json=standard_payload, headers=req_headers, timeout=REQUEST_TIMEOUT_API, verify=False, proxies={'http': None, 'https': None})
