...
try:
    from core.token_store import save_account_token, save_current_tokens
except ModuleNotFoundError:
    # 兼容服务器尚未同步 token_store.py 的旧部署
    ...
    def save_account_token(mail, token_val):
        try:
            ...
            return False, f"写入失败: {TOKEN_DIR} ({e})"
...
def run_login_check(task_config=None, custom_accounts=None):
    ...
    for account in accounts:
        ...
        try:
            ...
            if check_res["success"]:
                ...
                ok, err = save_account_token(mail, token_val)
                if not ok:
                    token_save_errors.append(err)
                    results_summary.append(f"{mail}:OK(登录成功,token未落盘)")
                else:
    ...
    if all_current_tokens:
        ok, err = save_current_tokens(all_current_tokens)
        if not ok:
            token_save_errors.append(err)

    # --- 汇总结果 ---
    total = len(accounts)
    login_api_ok = success_count == total and total > 0
    token_persist_ok = not token_save_errors
    final_result = {
        "success": login_api_ok and token_persist_ok,
        "message": f"【{ENV_TYPE}】" + " | ".join(results_summary),
        "status_code": last_status_code,
        "token": last_valid_token,
    }
    if token_save_errors:
        final_result["message"] += " | token保存失败:" + "; ".join(token_save_errors[:2])
        final_result["token_save_error"] = token_save_errors[0]
