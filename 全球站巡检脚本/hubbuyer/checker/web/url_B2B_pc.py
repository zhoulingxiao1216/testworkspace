# -*- coding: utf-8 -*-
# d:\sakuradk3\checker\web\url_B2B_pc.py

#五、url_B2B_pc.py 修改断言逻辑
import requests
import sys
import json
import os
...
def run(config, logger=None):
    """
    执行校验主逻辑
    :param logger: 传入 None 则不记录重复日志，传入实例则记录
    """
    url = config.get("url")
    task_name = config.get("desc") or "B2B_PC站点巡检"
    ...
    try:
        headers = {'User-Agent': 'Mozilla/5.0 Sakuradk2-Checker/1.0'}
