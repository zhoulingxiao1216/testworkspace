...
def get_b2b_pay_headers(api_base_url, b2b_site_url, token, cookie_str,
                        currency='USD', language='korean', nation='Korea',
                        rate='0.16', logintype='user'):
    """B2B 报价单支付专用请求头（对齐 quote/pay curl）"""
    headers = get_auth_headers(api_base_url, token)
    b2b_site_url = (b2b_site_url or api_base_url).rstrip('/')
    headers.update({
        'accept': 'application/json, text/plain, */*',
        'accept-language': 'zh-CN,zh;q=0.9',
        'content-type': 'application/json',
