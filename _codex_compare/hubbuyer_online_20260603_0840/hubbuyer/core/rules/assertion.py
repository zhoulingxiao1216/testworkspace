...
class AssertionTool:
    ...
    def verify_api_common(response, rules=None):
        ...
        try:
            ...
            # 4. 默认检查：如果没有json_match规则，则默认检查code字段是否为200（双200验证）
            if not (rules and "json_match" in rules):
                # 默认双200验证：HTTP status_code=200 且 JSON code=200
                actual_code = res_data.get("code")
                if actual_code is not None and actual_code != 200:
                    return {
                        "success": False,
                        "message": f"JSON code字段不匹配: 期望200，实际{actual_code}",
                        "expected": "code=200",
                        "actual": f"code={actual_code}"
                    }
