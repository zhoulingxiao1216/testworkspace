                        ...
                        try:
                            with open(orderid_file, "r", encoding="utf-8") as f:
                                ...
                                if target_mail in orderid_mapping:
                                    b2b_quote_nos = orderid_mapping[target_mail].get("B2B", [])
                        except:
                            pass
                    
                    if not b2b_quote_nos:
                        msgs.append("B2B报价单支付:FAIL(未找到报价单号)")
                    else:
                        ...
                        # 使用新的请求参数格式：{"quote_no_arr":"B2B-BJ-JPN25-260130-013"}
                        b2b_pay_payload = payment_payloads.get("B2B_pay", {}).copy()
                        # 使用第一个quote_no值
                        b2b_pay_payload["quote_no_arr"] = b2b_quote_nos[0]
                        
                        response = requests.post(b2b_pay_url, json=b2b_pay_payload, headers=b2b_pay_headers, timeout=REQUEST_TIMEOUT_API, verify=False, proxies={'http': None, 'https': None})
