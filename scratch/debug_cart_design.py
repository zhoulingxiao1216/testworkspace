import requests, json

headers = {
    'authorization': 'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..s-OurAj0yooRFDNYWczUjry8AZb2u7fFw6zeB0lWkXY',
    'userlogintoken': 'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..BUMfHiBm2WzoqTOdptC0kFDIOHsMbBm8vAu2jS-_dDk',
    'content-type': 'application/json'
}

resp = requests.post('https://api.hubbuyer.com/api_b2b/cart/list', json={}, headers=headers)
data = resp.json()

for seller in data.get('data', {}).get('seller_data', []):
    for item in seller.get('data', []):
        # Let's see if this item has design services
        keys = list(item.keys())
        design_keys = [k for k in keys if 'design' in k or 'tag' in k or 'brand' in k or 'fjx' in k]
        
        has_extra = False
        extra_data = {}
        for k in design_keys:
            if item.get(k):
                has_extra = True
                extra_data[k] = item.get(k)
                
        if extra_data:
            print(f"ID: {item.get('id')}, ItemID: {item.get('item_id')}")
            for k, v in extra_data.items():
                if k != 'check_fjx_remark' and k != 'check_fjx':
                    print(f"  {k}: {str(v)[:100]}")
